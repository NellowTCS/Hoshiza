...
function blockScroll(e: TouchEvent): void {
  ...
  if (t) {
    ...
    if (
      !dragged &&
      Math.abs(t.clientX - startX) + Math.abs(t.clientY - startY) > 4
    ) {
      dragged = true;
    }
  }
...
export function longPressDrag(node: HTMLElement): { destroy: () => void } {
  function onPointerDown(e: PointerEvent): void {
    if (drag.repoId || (e.pointerType !== "touch" && e.pointerType !== "pen"))
      return;
  ...
  function onPointerMove(e: PointerEvent): void {
    ...
    if (active) {
      ...
      if (
        !dragged &&
        Math.abs(e.clientX - startX) + Math.abs(e.clientY - startY) > 4
      ) {
        dragged = true;
      }
      return;
